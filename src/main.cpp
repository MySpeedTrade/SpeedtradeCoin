...
// Copyright (c) 2013-2014 The SpeedtradeCoin developers
...
#if defined(NDEBUG)
# error "SpeedtradeCoin cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "SpeedtradeCoin Signed Message:\n";
...
//TODO: this isn't identical to speedtradecoin reference client.
...
// Genesis block
      ...
      nSubsidy >>= ((nHeight - 1600000 + 200000) / 1000);
    }
    else
    {
        nSubsidy = 1 * COIN;
...
static const int64_t nTargetTimespan = 10 * 30; // SpeedtradeCoin: every 5min
static const int64_t nTargetTimespanNEW = 30 ; // SpeedtradeCoin: every 30sec
static const int64_t nTargetSpacing = 30; // SpeedtradeCoin: 30sec 
...
unsigned int GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // SpeedtradeCoin: This fixes an issue where a 51% attack can change difficulty at will.
...
void ThreadScriptCheck() {
    RenameThread("speedtradecoin-scriptch");
...
bool ConnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck)
{
    ...
    // BIP16 was always active in SpeedtradeCoin
...
bool AcceptBlockHeader(CBlockHeader& block, CValidationState& state, CBlockIndex** ppindex)
{
    ...
    if (hash != Params().HashGenesisBlock()) {
        ...
        // SpeedtradeCoin: reject v2 and v1 blocks at the same time, only check once
        if (block.GetBaseVersion() < 3) {
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp)
{
    ...
    // SpeedtradeCoin: reject ONLY if block.nVersion=3 has a supermajority because CBlockIndex::IsSuperMajority
