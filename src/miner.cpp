...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    if (auxpow != NULL) {
        ...
        // print to log
        LogPrintf("SpeedtradeCoinMiner: AUX proof-of-work found; our hash: %s ; parent hash: %s ; target: %s\n",
               hash.GetHex().c_str(),
    ...
    else
    {
        ...
        // print to log
        LogPrintf("SpeedtradeCoinMiner: proof-of-work found; hash: %s ; target: %s\n", hash.GetHex().c_str(), hashTarget.GetHex().c_str());
    }
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("SpeedtradeCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("SpeedtradeCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static SpeedtradeCoinMiner(CWallet *pwallet)
{
    LogPrintf("SpeedtradeCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("speedtradecoin-miner");
    ...
    try { while (true) {
        ...
        LogPrintf("Running SpeedtradeCoinMiner with %" PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        LogPrintf("SpeedtradeCoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet, int nThreads)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&SpeedtradeCoinMiner, pwallet));
}
