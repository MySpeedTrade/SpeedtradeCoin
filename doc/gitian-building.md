...
*Setup instructions for a gitian build of SpeedtradeCoin using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the SpeedtradeCoin
...
- [Building SpeedtradeCoin](#building-speedtradecoin)
...
Clone the git repositories for speedtradecoin and gitian,

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/speedtradecoin/speedtradecoin
```
...
In [doc/release-process.md](release-process.md) in the speedtradecoin repository under 'Fetch and build inputs'.
...
Building SpeedtradeCoin
----------------

To build SpeedtradeCoin (for Linux and/or Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the speedtradecoin repository.

Output from `gbuild` will look something like

    Initialized empty Git repository in /home/debian/gitian-builder/inputs/speedtradecoin/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/speedtradecoin/speedtradecoin
...
For example:
```bash
URL=https://github.com/langerhans/speedtradecoin.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit speedtradecoin=${COMMIT} --url speedtradecoin=${URL} ../speedtradecoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit speedtradecoin=${COMMIT} --url speedtradecoin=${URL} ../speedtradecoin/contrib/gitian-descriptors/gitian-win.yml
```
...
```bash
    gpg --detach-sign ${VERSION}/${SIGNER}/speedtradecoin-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/speedtradecoin-build.assert
```
...
[speedtradecoin/gitian.sigs](https://github.com/speedtradecoin/gitian.sigs/) repository, or
