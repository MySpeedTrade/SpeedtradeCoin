UNIX BUILD NOTES
====================
Some notes on how to build SpeedtradeCoin in Unix. 
...
This will build speedtradecoin-qt as well if the dependencies are met.
...
memory available when compiling SpeedtradeCoin Core. With 512MB of memory or less
...
If you want to build SpeedtradeCoin-Qt, make sure that the required packages for Qt development
...
Once these are installed, they will be found by configure and a speedtradecoin-qt executable will be
built by default.

Notes
-----
The release is built with GCC and then "strip speedtradecoind" to strip the debug
...
# Pick some path to install BDB to, here we create a directory within the speedtradecoin directory
...
# Configure SpeedtradeCoin Core to use our own-built instance of BDB
cd $BITCOIN_ROOT
...
cryptography functions, and therefore cannot be used for SpeedtradeCoin (or other
...
Security
--------
To help make your speedtradecoin installation more secure by making certain attacks impossible to
...
* Non-executable Stack
    ...
    vulnerable buffers are found. By default, speedtradecoin should be built with a non-executable stack
...
Disable-wallet mode
--------------------
When the intention is to run only a P2P node without a wallet, speedtradecoin may be compiled in
